...
class Rjn0003ForkJoinJobnetBuilder < JobnetFixtureBuilder
  DSL = <<-EOS
    jobnet("rjn0003") do
      boot_jobs("j11", "j12")
      job("j11", "$HOME/j11.sh", :to => "j13")
      job("j12", "$HOME/j12.sh", :to => ["j15", "j17"])
      job("j13", "$HOME/j13.sh", :to => ["j14", "j17"])
      job("j14", "$HOME/j14.sh", :to => "j16")
      job("j15", "$HOME/j15.sh")
      job("j16", "$HOME/j16.sh")
      job("j17", "$HOME/j17.sh")
    end
  ...
  def create(options = {})
    ...
    root.children << new_script("j13", :script => "$HOME/j13.sh")
    root.children << new_fork # F2
    root.children << new_fork # F3
    root.children << new_join # J1
    root.children << new_script("j14", :script => "$HOME/j14.sh")
    root.children << new_script("j15", :script => "$HOME/j15.sh")
    root.children << new_script("j16", :script => "$HOME/j16.sh")
    root.children << new_script("j17", :script => "$HOME/j17.sh")
