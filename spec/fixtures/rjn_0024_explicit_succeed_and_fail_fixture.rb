...
class Rjn0024ExplicitSucceedAndFailFixture < JobnetFixtureBuilder
  ...
  DSL = <<-EOS
    jobnet("rjn0024") do
      ...
      # job.success の後に job.failure を実行するとerrorに。
      ruby_job('j11', :conductor => lambda{|job| job.run; job.succeed; job.fail}){|job| STDOUT.puts("j11 end") }

      # job.fail の後に job.succeed を実行するとsuccessに。
      ruby_job('j12', :conductor => lambda{|job| job.run; job.fail; job.success}){|job| STDOUT.puts("j12 end") }
    end
  EOS
