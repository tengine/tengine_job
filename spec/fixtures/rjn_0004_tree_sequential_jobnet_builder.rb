# -*- coding: utf-8 -*-
# 以下のジョブネットについてテンプレートジョブネットや
# 実行用ジョブネットを扱うフィクスチャ生成のためのクラスです。
#
# in [rjn0004]
# [S1] --e1-->[j1100]--e2-->[j1200]--e3-->[j1300]--e4-->[j1400]--e5-->[j1500]--e6-->[j1600]--e7-->[E1]
#
# [j1100]
# [S2]--e8-->[j1110]--e9-->[j1120]--e10-->[E2]
#
# [j1200]
# [S3]--e11-->[j1210]--e12-->[E3]
#
# [j1300]
# [S4]--e13-->[j1310]--e14-->[E4]
#
# [j1400]
# [S5]--e15-->[j1410]--e16-->[E5]
#
# [j1500]
# [S6]--e17-->[j1510]--e18-->[E6]
#
# [j1510]
# [S7]--e19-->[j1511]--e20-->[E7]
#
# [j1600]
# [S8]--e21-->[j1610]--e22-->[j1620]--e23-->[j1630]--e24-->[E8]
#
# [j1610]
# [S9]--e25-->[j1611]--e26-->[j1612]--e27-->[E9]
#
# [j1620]
# [S10]--e28-->[j1621]--e29-->[E10]
#
# [j1630]
# [S11]--e30-->[j1631]--e31-->[E11]
class Rjn0004TreeSequentialJobnetBuilder < JobnetFixtureBuilder
  DSL = <<-EOS
    jobnet("rjn0004") do
      jobnet("j1100", :credential_name => "goku_ssh_pw", :server_name => "hadoop_master_node") do
        job("j1110", "$HOME/j1110.sh")
        job("j1120", "$HOME/j1120.sh")
      end
      jobnet("j1200", :credential_name => "goku_ssh_pw") do
        job("j1210", "$HOME/j1210.sh", :server_name => "mysql_master")
      end
      jobnet("j1300", :server_name => "mysql_master") do
        job("j1310", "$HOME/j1310.sh", :credential_name => "goku_ssh_pw")
      end
      jobnet("j1400") do
        job("j1410", "$HOME/j1410.sh", :credential_name => "goku_ssh_pw", :server_name => "mysql_master")
      end
      jobnet("j1500", :credential_name => "goku_ssh_pw", :server_name => "mysql_master") do
        jobnet("j1510") do
          job("j1511", "$HOME/j1511.sh")
        end
      end
      jobnet("j1600", :credential_name => "goku_ssh_pw", :server_name => "mysql_master") do
        jobnet("j1610") do
          job("j1611", "$HOME/j1611.sh", :server_name => "hadoop_master_node") # server_nameを上書き
          job("j1612", "$HOME/j1612.sh", :credential_name => "gohan_ssh_pk") # credential_nameを上書き
        end
        jobnet("j1620", :server_name => "hadoop_master_node") do # server_nameを上書き
          job("j1621", "$HOME/j1621.sh")
        end
        jobnet("j1630", :credential_name => "gohan_ssh_pk") do # credential_nameを上書き
          job("j1631", "$HOME/j1631.sh")
        end
      end
    end
  EOS

  def create(options = {})
    resource_fixture = GokuAtEc2ApNortheast.new
    resource_fixture.goku_ssh_pw
    resource_fixture.hadoop_master_node
    resource_fixture.mysql_master

    root = new_root_jobnet("rjn0004", options)
    root.children << new_jobnet("j1100", :credential_name => "goku_ssh_pw", :server_name => "hadoop_master_node")
    root.children << new_jobnet("j1200", :credential_name => "goku_ssh_pw")
    root.children << new_jobnet("j1300", :server_name => "mysql_master")
    root.children << new_jobnet("j1400")
    root.children << new_jobnet("j1500", :credential_name => "goku_ssh_pw", :server_name => "mysql_master")
    root.children << new_jobnet("j1600", :credential_name => "goku_ssh_pw", :server_name => "mysql_master")
    root.prepare_end
    root.build_sequencial_edges

    j1100 = self[:j1100]
    j1100.children << new_script("j1110", :script => "$HOME/j1110.sh")
    j1100.children << new_script("j1120", :script => "$HOME/j1120.sh")
    j1100.prepare_end
    j1100.build_sequencial_edges

    j1200 = self[:j1200]
    j1200.children << new_script("j1210", :script => "$HOME/j1210.sh", :server_name => "mysql_master")
    j1200.prepare_end
    j1200.build_sequencial_edges

    j1300 = self[:j1300]
    j1300.children << new_script("j1310", :script => "$HOME/j1310.sh", :credential_name => "goku_ssh_pw")
    j1300.prepare_end
    j1300.build_sequencial_edges

    j1400 = self[:j1400]
    j1400.children << new_script("j1410", :script => "$HOME/j1410.sh", :server_name => "mysql_master", :credential_name => "goku_ssh_pw")
    j1400.prepare_end
    j1400.build_sequencial_edges

    j1500 = self[:j1500]
    j1500.children << new_jobnet("j1510")
    j1500.prepare_end
    j1500.build_sequencial_edges

    j1510 = self[:j1510]
    j1510.children << new_script("j1511", :script => "$HOME/j1511.sh")
    j1510.prepare_end
    j1510.build_sequencial_edges

    j1600 = self[:j1600]
    j1600.children << new_jobnet("j1610")
    j1600.children << new_jobnet("j1620", :server_name => "hadoop_master_node")
    j1600.children << new_jobnet("j1630", :credential_name => "gohan_ssh_pk")
    j1600.prepare_end
    j1600.build_sequencial_edges

    j1610 = self[:j1610]
    j1610.children << new_script("j1611", :script => "$HOME/j1611.sh", :server_name => "hadoop_master_node")
    j1610.children << new_script("j1612", :script => "$HOME/j1612.sh", :credential_name => "gohan_ssh_pk")
    j1610.prepare_end
    j1610.build_sequencial_edges

    j1620 = self[:j1620]
    j1620.children << new_script("j1621", :script => "$HOME/j1621.sh")
    j1620.prepare_end
    j1620.build_sequencial_edges

    j1630 = self[:j1630]
    j1630.children << new_script("j1631", :script => "$HOME/j1631.sh")
    j1630.prepare_end
    j1630.build_sequencial_edges

    root.save!
    root
  end
end
